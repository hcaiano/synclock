...
## Theme
Primary context is a **dark vibrancy popover** (producers work in dim studios / on stage). A light-appearance variant follows the same tokens via macOS semantic colors. Color strategy: **Restrained** — neutral surfaces + a single `#16C79A` Mint accent reserved for active/selected/primary and the live pulse. Nothing decorative.
...
| `--accent` | `oklch(0.74 0.14 168)` (#16C79A, Mint) | active sync, selected mode, primary, pulse |
