...
| 9 | **Packaging + distribution** | build+DMG+Sparkle wiring ✅ / sign+notarize BLOCKED | `Scripts/build-app.sh` → launchable `Synclock.app` (Info.plist LSUIElement, B `Synclock.icns`, Sparkle.framework bundled, hardened runtime). `Scripts/make-dmg.sh` → drag-to-install `Synclock-<v>.dmg` and signs the DMG when a Developer ID identity is present. `Scripts/notarize.sh` notarizes/staples `.app` or `.dmg` and fails fast unless the artifact is Developer-ID signed. Sparkle is linked in `SynclockApp`; release builds inject `SUFeedURL` + `SUPublicEDKey` via `SPARKLE_PUBLIC_ED_KEY`. BLOCKED only on Henrique's Apple Developer ID and on an EdDSA key + appcast host for the first live update feed. Full runbook in RELEASING.md. |
| 10 | **Brand + marketing site** | ✅ core complete | `site/index.html` static landing page (register=brand) reusing the real popover render + B icon: hero, the-problem (with the old-jitter code story), 3-step how-it-works, features (incl. live jitter stat + Link modes), download, free+BMC, GPLv2 footer. Passed impeccable design hook. Lineup-style Cloudflare Workers static-assets config (`site/wrangler.toml`, `_headers`, robots, sitemap, README). **DEPLOYED LIVE: `synclock` Worker on the Caiano account, custom domain `synclock.caiano.com` attached (DNS auto-created, HTTP 200, cert provisioned). Mirrors Lineup's lineup.caiano.com pattern.** Remaining: OG card (done), optional pulse demo. |
...
## Status: BUILDING — ~92% of v1 done & verified. Pushed to github.com/hcaiano/synclock (main). Phases 0-8 and 10 core-complete; Phase 9 scaffolding now includes Sparkle app wiring, but final notarized DMG/appcast remains BLOCKED on Henrique's Apple Developer ID + Sparkle EdDSA key. 83 unit checks green; Phase 5 Follow/Lead proof green; jitter p99 ~0.05ms @120 BPM. Open: Dev ID; deploy site to synclock.caiano.com; full test-gear list; external Ableton Live smoke test.
