...
| 5 | **Ableton Link integration** | ✅ core complete | `LinkFollowGrid` derives the MIDI tick grid from Link tempo+phase through a sampled `LinkClockMapper`; Free disables Link; Follow joins Link read-only and adopts peer tempo/start-stop; Lead keeps the local FreeRunningGrid and commits tempo/start-stop to Link. Verified: `swift run SynclockFollowCheck` (peer discovery, Follow tempo+transport, Lead tempo+transport), `swift run SynclockLinkCheck --self-peer`, 83 unit checks. Remaining: Claude follow-mode jitter pass + external Ableton Live peer smoke test. |
...
| 9 | **Packaging + distribution** | scaffolding ✅ / notarize BLOCKED | `Scripts/build-app.sh` assembles `Synclock.app` (Info.plist LSUIElement, B `Synclock.icns` via iconutil, hardened runtime, ad-hoc sign) — builds + launches as a menubar agent. BLOCKED on Henrique's Apple Developer ID: Developer-ID signing, notarize+staple, DMG, Sparkle EdDSA key + appcast host. Script is parameterized (IDENTITY env) so it's one step once the account is live. |
