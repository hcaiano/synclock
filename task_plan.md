...
| 10 | **Brand + marketing site** | pending | Static site at **synclock.caiano.com** (register=brand) reusing the real popover + Devices widgets for a "How it works" section. Hero, problem, 3-step how-it-works, features, timing proof, download (DMG/GitHub), free+BMC, GPL link. Assets from locked B identity: icon, logo/wordmark, favicon, OG/social images, screenshots, optional looping pulse demo. |
...
## Status: PLAN FINAL & SIGNED OFF — grill done; v1 design + gear model + UI/UX locked; GOAL_PROMPT.md carries Codex's 5 edits; icon = B "Pulse Path" (founder signed off); marketing site (synclock.caiano.com) + Phase 10 scoped. READY: Henrique pastes GOAL_PROMPT.md to both agents → Phase 0. Only non-blocking open: real test-gear list (pin at Phase 8).
