...
- **Distribution:** notarized DMG, hardened runtime, Developer ID signing, **Sparkle** appcast, GitHub Releases. App icon/menubar glyph/wordmark use the locked Mint Pulse Tile identity from Codex.
...
- **Brand/logo direction (LOCKED — founder signed off):** founder replaced the earlier Tempo Dot / Pulse Coral direction with **A "Mint Pulse Tile"** from `branding/explorations-v3/a-mint-pulse-tile`: a fresh mint/emerald tile with a simple, friendly pulse mark. Accent is **Fresh Mint `#16C79A`** (distinct from Lineup blue and the retired coral). Production `.appiconset`, menubar template glyph, favicon, wordmark, site assets, and OG image derive from `branding/explorations-v3/a-mint-pulse-tile` via `branding/generate_synclock_icons.py`.
...
9. Packaging (notarized DMG + Sparkle + GitHub Releases + locked Mint Pulse Tile identity integration).
10. **Brand + marketing site** (`synclock.caiano.com`): build the static landing page reusing the real widget UI for "How it works," plus favicon, logo/wordmark, social/OG assets, screenshots, and optional pulse demo from the locked Mint Pulse Tile identity.
